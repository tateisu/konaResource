...
open class KonaSha256Benchmark {
    ...
    fun setup() {
        ...
        files.forEach { input ->
            val defaultDigest = digest(input, defaultSha256())
            val kotlinDigest = digest(input, KonaSha256Kotlin())
            check(defaultDigest.contentEquals(kotlinDigest)) {
                "SHA-256 implementations returned different results"
            }
        }
    }
    ...
    @Benchmark
    open fun pureKotlinImplementation(): Int = digestAll { KonaSha256Kotlin() }
