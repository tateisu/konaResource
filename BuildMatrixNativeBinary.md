...
`common`のBLAKE3実装は、x64ターゲットではSSE2・SSE4.1・AVX2・AVX512の実装を含み、実行時にCPU機能を判定して使用する。
ただしWindows x64ではBLAKE3実装側の制約により、AVX512は一部の経路で無効化される。そのため、Windows x64はLinux/macOS x64と完全に同一の最適化経路ではない。
arm64ターゲットではNEON実装を使用する。
